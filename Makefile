...
YOURLS_SHORTURL=yourls/shorturl
YOURLS_INIT=yourls/init
BEAGLE_SCAFFOLD_ANDROID=beagle/scaffold/android
UTILS_REPLACE_KEYVALUE=utils/replace/keyvalue
BEAGLE_SCAFFOLD_WEB=beagle/scaffold/web
BEAGLE_SCAFFOLD_BACKEND=beagle/scaffold/backend
FORMULAS=$(TERRAFORM_APPLY) $(SC_COFFEE_GO) $(SC_COFFEE_JAVA) $(SC_COFFEE_SHELL) $(SC_COFFEE_PYTHON) $(SC_COFFEE_NODE) $(SC_SPRING_STARTER) $(KAFKA) $(DOCKER) $(NAVIGATE_HANDBOOK) $(SEARCH_HANDBOOK) $(KUBERNETES) $(FAST_MERGE) $(AWS_SETCONTEXT) $(RITCHIE_TEAM) $(FILES_RITCHIE_TEAM) $(KUBERNETES_CLEANER) $(AWS_BUCKET) $(TERRAFORM_AWS_PROJECT) $(TERRAFORM_AWS_VPC) $(TERRAFORM_AWS_EKS) $(CIRCLECI_ADD_ENV) $(GITHUB_UPDATE_FORK) $(GITHUB_CREATE_REPO) $(GITHUB_ADD_COLLABORATOR) $(JUPYTER_CREATE_ML_TEMPLATE) $(YOURLS_SHORTURL) $(YOURLS_INIT) $(BEAGLE_SCAFFOLD_ANDROID) $(UTILS_KEYPAIR_REPLACE) $(BEAGLE_SCAFFOLD_BACKEND) $(BEAGLE_SCAFFOLD_WEB)
